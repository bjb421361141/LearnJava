...
#### 远程仓库免密 使用ssh token 进行操作
- token创建和使用
  1. 使用 `ssh-keygen -t rsa -C "youremail@example.com"` 创建token。
  2. 复制 `.pub` 文件中的内容至github ssh操作目录下
- 本地配置
  1. 使用`git remote -v `查看仓库使用的推送和接收方式 应使用ssh模式
  ```
...
```
#### 查看分支合并图
可以使用 --pretty=oneline 和 --abbrev-commit 进行输出简化输出内容
```bash
$ git log --graph
```
...
#### 创建自己的git Repository
- 创建git使用用户
  1. 使用 `useradd mygit` 创建用户 并到 `\etc\passwd` 文件中修改 登入使用的shell 为/usr/bin/git-shell。
  2. 在 `/home/mygit` 下创建 ` .ssh\authorized_keys` 文件并往文件中添加用户的公钥
- 创建仓库
  1. 使用`git init --bare sample.git`创建一个裸仓库
  2. 更改权限 `chown -R mygit:mygit sample.git`
- 客户端使用
  1. 使用$ git clone mygit@server:/home/mygit/sample.git
  2. 添加完文件直接提交就会生成一个master 主版本，并推送至仓库
