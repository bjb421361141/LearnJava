        ...
        for (Serie serie : series) {
            ...
            if (data != null && categories != null && data.size() == categories.length) {
                for (int index = 0; index < data.size(); index++) {
                    ...
                    if (isNumber(value)) {
                        //name 为纵坐标   categories[index] 为横坐标
...
//           frame.setLocationRelativeTo(null);
        try {
            ...
            ChartPanel chartPanel = new CreatLineChart().createChart(NO_DATA_MSG, NO_DATA_MSG, NO_DATA_MSG, Arrays.asList(categories), series);
//            frame.getContentPane().add(chartPanel);
//            frame.setVisible(true);
            //将图片保存为png格式
//            saveAsFile(chartPanel.getChart(),"D:\\1\\lol.png",900,500);
            CreateNewLineChartForPng("运费价格表", "运费", "价格", "C:\\Users\\Baijb\\Desktop\\运费.png", Arrays.asList(categories), series, 900, 500);
